...
def get_scrap_movie_detail(movie_list):
	if os.path.exists("task250.json"):
		with open ("task250.json",'r') as file:
			read=file.read()
			row=json.loads(read)
			return(row)
	else:
		...
		for i in movie_list:
			...
			row.append(m_data)
		with open("task250.json","w") as file:
			read = json.dumps(row)
			file.write(read)
			file.close()
	return(row)
detail=get_scrap_movie_detail(top_movies[:249])
def director_detail(director_data):
	directors={}
	direct=[]
	for i in detail:
		for j in i["Director"]:
	...
	for i in movie_list:
		for director in dic_director:
			if director in i['Director']:
				for language_ in i['Language']:
					if language_ not in dic_director[director]:
						dic_director[director][language_]=1
					else:
						dic_director[director][language_] +=1
	return(dic_director)
pprint(analyses_language_and_director(detail))
