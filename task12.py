...
def scrap_movie_cast(movie_cast_url):
	if os.path.exists("movie_all_cast.json"):
		with open ("movie_all_cast.json",'r') as file:
			read=file.read()
			main_data=json.loads(read)
			return(main_data)
	else:
		...
		for i in movie_cast_url:
			...
			for i in tbody:
				...
				for j in td:
					...
					"name":name}
					c_data.append(dict1)
			cast_data.append(c_data)
			with open("movie_all_cast.json","w") as file:
					read1 = json.dumps(cast_data)
					file.write(read1)
					file.close()
		return(cast_data)
